...
import { ApiOperation, ApiOkResponse, ApiQuery, ApiParam, ApiBearerAuth, ApiTags } from '@nestjs/swagger';
...
export class BoardsController {
  ...
  update(
    ...
    return this.boardsService.update(id, updateBoardDto, req.user.id, req.user.role);
  }
  ...
  addMember(
    ...
    return this.boardsService.addMember(id, req.user.id, body.userId, body.role, req.user.role);
  }
  ...
  removeMember(
    ...
    return this.boardsService.removeMember(id, req.user.id, userId, req.user.role);
  }
  ...
  createLabel(
    ...
    return this.boardsService.createLabel(id, body.name, body.color, req.user.id, req.user.role);
  }
  ...
  getActivities(
    ...
    return this.boardsService.getActivities(id, req.user.id, { page: p, pageSize: ps });
  }

  @Get(':id/activities/export')
  @ApiOperation({ summary: 'Export board activities as CSV' })
  @ApiParam({ name: 'id', description: 'Board ID' })
  @ApiQuery({ name: 'format', required: false, description: 'Export format', enum: ['csv'], example: 'csv' })
  ...
  async exportActivities(
    ...
    const csv = await this.boardsService.exportActivitiesCsv(id, req.user.id, { cardId, userId });
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename=board-${id}-activities-${Date.now()}.csv`);
    return res.send(csv);
  }
}
