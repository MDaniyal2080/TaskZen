...
export class BoardsService {
  ...
  async create(createBoardDto: CreateBoardDto, userId: string) {
    ...
    const MAX_FREE_BOARDS = Number((settings as any)?.general?.maxBoardsPerUser ?? 3);
    ...
    if (createBoardDto.templateId) {
      ...
      template = builtInTemplates.find(t => t.id === createBoardDto.templateId);
    ...
    const { templateId, ...boardData } = createBoardDto;
    ...
    if (!publicBoardsEnabled && boardData?.isPrivate === false) {
      throw new BadRequestException('Public boards are disabled by the administrator');
    }
  ...
  async update(id: string, updateBoardDto: UpdateBoardDto, userId: string, userRole?: string) {
    ...
    const canUpdate = board.ownerId === userId || 
      (member && ([BoardMemberRole.OWNER, BoardMemberRole.ADMIN] as BoardMemberRole[]).includes(member.role));
  ...
  async addMember(boardId: string, userId: string, memberUserId: string, role: BoardMemberRole = BoardMemberRole.MEMBER, userRole?: string) {
    const board = await this.findOne(boardId, userId, userRole);
    
    // Check if user has permission to add members
    const member = board.members.find(m => m.userId === userId);
    const canAddMembers = board.ownerId === userId || 
      (member && ([BoardMemberRole.OWNER, BoardMemberRole.ADMIN] as BoardMemberRole[]).includes(member.role));
  ...
  async removeMember(boardId: string, userId: string, memberUserId: string, userRole?: string) {
    const board = await this.findOne(boardId, userId, userRole);
    
    // Check if user has permission to remove members
    const member = board.members.find(m => m.userId === userId);
    const canRemoveMembers = board.ownerId === userId || 
      (member && ([BoardMemberRole.OWNER, BoardMemberRole.ADMIN] as BoardMemberRole[]).includes(member.role));
  ...
  async exportActivitiesCsv(
    ...
    const rows = activities.map((a) => {
      const description = buildDescription((a as any)?.type, (a as any)?.data ?? {});
      const userLabel = (a as any)?.user?.firstName || (a as any)?.user?.username || '';
      const metadata = JSON.stringify((a as any)?.data ?? {});
      return [
        a.id,
        a.boardId || '',
        a.cardId || '',
        toDotType((a as any)?.type),
        description,
        userLabel,
        a.userId || '',
  ...
  async saveAsTemplate(boardId: string, userId: string) {
    // Fetch board with lists and cards for structure capture
    const board = await this.prisma.board.findUnique({
      where: { id: boardId },
      include: {
        members: true,
    ...
    // Permission: owner or admin-level member can save as template
    const member = board.members.find((m) => m.userId === userId);
    const canSave =
      board.ownerId === userId ||
      (member && ([BoardMemberRole.OWNER, BoardMemberRole.ADMIN] as BoardMemberRole[]).includes(member.role));
    ...
    // Build minimal structure for template (lists and cards only)
    const structure = {
    ...
    const created = await this.prisma.boardTemplate.create({
      data: {
        name: board.title,
    ...
    // Return shape compatible with client template interface
    return {
      id: created.id,
      name: created.name,
  ...
  async listTemplates(userId: string) {
    ...
    const builtInTemplates = [
      ...
      },
      {
        ...
        description: 'Complete project workflow board',
        icon: 'FolderKanban',
        color: '#10b981',
        lists: [
          { title: 'Backlog', position: 1, cards: [] },
    ...
    const customMapped = customTemplates.map((t) => ({
      ...
      icon: t.icon,
      color: t.color,
      lists: (t.structure as any)?.lists || [],
      isCustom: t.isCustom,
    }));

    return [...builtInTemplates, ...customMapped];
  }

  async createLabel(boardId: string, name: string, color: string, userId: string, userRole?: string) {
    ...
    if (existing) {
      throw new BadRequestException('Label with this name already exists on this board');
    }
