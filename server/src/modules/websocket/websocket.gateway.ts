...
// Strongly-typed CORS origin function for Socket.IO
const corsOrigin: (
  origin: string | undefined,
  callback: (err: Error | null, allow: boolean) => void,
) => void = (origin, callback) => {
  const envOrigins = [process.env.FRONTEND_URL, process.env.CLIENT_URL].filter(
    Boolean,
  ) as string[];
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  private async isRealtimeEnabled(): Promise<boolean> {
    try {
      ...
      return Boolean(settings.features?.enableRealTimeUpdates);
    } catch {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      this.logger.log(
        `WS connect id=${client.id} nsp=${nsp} origin=${origin} query=${JSON.stringify(query)}`,
      );
    } catch (e) {
      this.logger.warn(`WS connect log failed: ${e?.message || e}`);
    }
    if (!(await this.isRealtimeEnabled())) {
      client.emit("realtimeDisabled", {
        message: "Real-time updates are disabled by the administrator",
      });
  ...
  async emitToBoardMembers(boardId: string, event: string, data: unknown) {
  ...
  // Board update events
  notifyBoardUpdate(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "boardUpdated", data);
  }

  notifyBoardDeleted(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "boardDeleted", data);
  }

  notifyListCreated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "listCreated", data);
  }

  notifyListUpdated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "listUpdated", data);
  }

  notifyListDeleted(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "listDeleted", data);
  }

  notifyCardCreated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "cardCreated", data);
  }

  notifyCardUpdated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "cardUpdated", data);
  }

  notifyCardMoved(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "cardMoved", data);
  }

  notifyCardDeleted(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "cardDeleted", data);
  }

  // Comment events
  notifyCommentCreated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "commentCreated", data);
  }

  notifyCommentUpdated(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "commentUpdated", data);
  }

  notifyCommentDeleted(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "commentDeleted", data);
  }

  notifyMemberAdded(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "memberAdded", data);
  }

  notifyMemberRemoved(boardId: string, data: unknown) {
    this.emitToBoardMembers(boardId, "memberRemoved", data);
  }

  // Activity events
  notifyActivityCreated(boardId: string, data: unknown) {
