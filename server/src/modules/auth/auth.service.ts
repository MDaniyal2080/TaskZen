import { Injectable, UnauthorizedException, ConflictException, ServiceUnavailableException, BadRequestException, HttpException, HttpStatus } from '@nestjs/common';
...
export class AuthService {
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    const requiredLen = Math.max(6, Number((settings as any)?.security?.passwordMinLength ?? 6));
    if ((dto.newPassword || '').length < requiredLen) {
      throw new BadRequestException(`Password must be at least ${requiredLen} characters long`);
    }

    // Use UsersService.update to handle hashing
    await this.usersService.update(user.id, { password: dto.newPassword } as any);
    return { success: true };
  }
  ...
  async login(loginDto: LoginDto) {
    ...
    const attemptsTtlSec = Math.max(60, Number(security?.loginAttemptWindowSec ?? 900)); // default 15m
    ...
    if ((settings as any)?.maintenance?.enabled && user.role !== 'ADMIN') {
      throw new ServiceUnavailableException('The service is under maintenance. Only admins can log in.');
    }
  ...
  public async generateToken(payload: { email: string; sub: string; role: string }) {
    const settings = await this.settingsService.getSettings();
    const minutes = Math.max(1, Number((settings as any)?.security?.sessionTimeout ?? 10080)); // default 7d
